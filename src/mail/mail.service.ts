...
export class MailService {
  private sgMail: SendGridMail;
  private from: string;

  constructor(private config: ConfigService) {
    this.sgMail = new SendGridMail();
    this.sgMail.setApiKey(this.config.get<string>('SENDGRID_API_KEY')!);
    this.from = this.config.get<string>('SENDER_EMAIL')!;
  }
  ...
  private header(bg: string, title: string) {
    return `
  ...
  async sendReferralLink(email: string, customerName: string, campaignName: string, code: string) {
  ...
  async sendReferrerRewardNotification(
